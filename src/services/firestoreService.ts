...
const mapCustomerDoc = (id: string, data: Record<string, unknown>): Customer => {
  ...
  return {
    ...
    customerType: data.customerType === 'medical' ? 'medical' : 'customer',
    branchId: data.branchId === 'MASKI' || data.branchId === 'SINDHANUR' ? data.branchId : undefined,
    tier,
...
const mapInvoiceDoc = (id: string, data: Record<string, unknown>): Invoice => {
  ...
  return {
    ...
    salesStaffDirectoryToken: data.salesStaffDirectoryToken ? String(data.salesStaffDirectoryToken) : undefined,
