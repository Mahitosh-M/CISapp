...
const InstallAppPrompt = ({ disabled = false }: { disabled?: boolean }) => {
  ...
  return (
    <div style={{ position: 'fixed', left: 12, right: 12, bottom: 86, zIndex: 15, display: 'grid', placeItems: 'center', pointerEvents: 'none' }}>
      <div style={{ width: '100%', maxWidth: 420, borderRadius: 22, overflow: 'hidden', background: '#FFFFFF', color: '#0F172A', boxShadow: '0 22px 45px rgba(6,27,58,0.24)', pointerEvents: 'auto', border: '1px solid rgba(212,175,55,0.35)' }}>
        <div style={{ background: 'linear-gradient(135deg, #0F172A 0%, #11185A 45%, #4C1D95 100%)', color: '#FFFFFF', padding: 14, display: 'flex', gap: 12, alignItems: 'center' }}>
          <img src="/icons/icon-192.png" alt="" style={{ width: 46, height: 46, borderRadius: 13, boxShadow: '0 8px 18px rgba(212,175,55,0.22)' }} />
          <div>
        ...
        <div style={{ padding: 15 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginTop: 14 }}>
            <button type="button" onClick={closePrompt} style={{ border: 0, borderRadius: 13, background: '#E8EDF4', color: '#0F172A', padding: 11, fontWeight: 900 }}>
              {isIosPrompt ? 'Later' : 'Later'}
            </button>
            <button type="button" onClick={isIosPrompt ? closePrompt : installApp} style={{ border: 0, borderRadius: 13, background: '#F6C343', color: '#0F172A', padding: 11, fontWeight: 900, display: 'inline-flex', justifyContent: 'center', alignItems: 'center', gap: 7 }}>
